...
# Predicción del Índice S&P 500 Utilizando Redes Neuronales Elman y Jordan
...
```{r message=FALSE, warning=FALSE, , include=TRUE, dev='png', dpi=96, include=FALSE}
...
```{r echo=TRUE, message=FALSE, warning=FALSE, , include=TRUE, dev='png', dpi=96}

library(caret)
spx_data_prepared$time_index <- seq_along(spx_data_prepared$ds)

head(spx_data_prepared$time_index)
set.seed(123)
trainIndex <- createDataPartition(spx_data_prepared$y, p = 0.8, list = FALSE)
trainData <- spx_data_prepared[trainIndex, ]
testData <- spx_data_prepared[-trainIndex, ]
```
...
head(trainData$y_scaled)
head(testData$y_scaled)

```

### Preparación de las Entradas y Salidas.
...
print(paste("RMSE de la Red Elman:", rmse_elman))
```
...
print(paste("RMSE Ajustado de la Red Elman:", rmse_elman_adjusted))

results_elman_adjusted <- data.frame(
  Fecha = testData$ds,
  Real = testData$y,
...
Entradas insuficientes: Usar únicamente el índice temporal como entrada puede no ser suficiente para modelar una serie temporal tan compleja como esta.
Limitaciones del modelo Elman: Esta arquitectura puede no ser adecuada para capturar patrones complejos y dependencias a largo plazo en los datos financieros.
...
print(paste("RMSE de la Red Jordan:", rmse_jordan))
...
Inadecuación de la arquitectura: La red Jordan puede no ser lo suficientemente robusta para modelar series temporales con alta variabilidad y tendencias a largo plazo.
Falta de características adicionales: Al igual que con Elman, no se incluyeron valores de retraso (lags) ni otras variables que puedan mejorar la capacidad del modelo para identificar patrones en los datos.
